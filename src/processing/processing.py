...
def process_task(task_path, db_info, config, run_seed, run_idx, task_idx):
    ...
    # Split data into train and test using the run-specific seed
    ...
    if hasattr(config, 'feature_selection') and config.feature_selection.enabled:
        ...
        # Create feature selection output directory if needed
        fs_output_dir = None
        ...
        if feature_selection_method == 'selectkbest':
            ...
            try:
                ...
                if selector:
                    ...
                    # Save feature selection results if configured
    ...
    # Create task-specific output directory with organized structure based on models
    ...
    results = run_hyperparameter_search(
        ...
        cv=cv,
        metric=metric,
        output_dir=task_output_dirs,  # Pass the dictionary with output dirs for each model
        run_seed=run_seed  # Pass the seed to ensure reproducibility in CV splits
    )
    
    # Find best model overall for display purposes
